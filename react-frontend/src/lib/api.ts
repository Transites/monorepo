...
async function authenticatedApiRequest<T>(
  ...
  // Ensure headers are a plain object so apiRequest can merge them with Content-Type
  const existingHeaders = (options.headers && typeof options.headers === 'object')
    ? (options.headers as any)
    : {};

  const headersObj = {
    ...((existingHeaders instanceof Headers) ? Object.fromEntries((existingHeaders as any).entries()) : existingHeaders),
...
export async function updateArticle(
  ...
  const response = await authenticatedApiRequest<{ submission: Submission }>(
    `/articles/${id}`,
    {
      method: 'PATCH',
